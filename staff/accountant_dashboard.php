...
<head>
    ...
    <style>
        /* ===== SAME STYLES AS OTHER STAFF DASHBOARDS ===== */
        * {
            margin: 0;
            padding: 0;
...
<div class="sidebar" id="sidebar">
    <div class="sidebar-header">
        ...
        <div class="row-tagline">
            <span class="tagline">Accountant Portal</span>
...
<div class="main-content">
    <div class="top-bar">
        <div class="welcome-message">Welcome, <span><?php echo htmlspecialchars($accountant_name); ?></span></div>
    ...
    <div class="dashboard-body">
        ...
        <?php if ($active_page == 'dashboard'): ?>
            ...
            mysqli_stmt_bind_param($stats_stmt, "i", $accountant_id);
            ...
            <div class="content-area">
                ...
                mysqli_stmt_bind_param($recent_stmt, "i", $accountant_id);
        ...
        <?php elseif ($active_page == 'complaints' || $active_page == 'pending' || $active_page == 'resolved' || $active_page == 'escalated'): ?>
            ...
            if ($priority_filter && $search) {
                $search_param = "%$search%";
                mysqli_stmt_bind_param($count_stmt, "iss", $accountant_id, $priority_filter, $search_param);
            } elseif ($priority_filter) {
                mysqli_stmt_bind_param($count_stmt, "is", $accountant_id, $priority_filter);
            } elseif ($search) {
                $search_param = "%$search%";
                mysqli_stmt_bind_param($count_stmt, "is", $accountant_id, $search_param);
            } else {
                mysqli_stmt_bind_param($count_stmt, "i", $accountant_id);
            }
            ...
            if ($priority_filter && $search) {
                $search_param = "%$search%";
                mysqli_stmt_bind_param($data_stmt, "issii", $accountant_id, $priority_filter, $search_param, $limit, $offset);
            } elseif ($priority_filter) {
                mysqli_stmt_bind_param($data_stmt, "isii", $accountant_id, $priority_filter, $limit, $offset);
            } elseif ($search) {
                $search_param = "%$search%";
                mysqli_stmt_bind_param($data_stmt, "isii", $accountant_id, $search_param, $limit, $offset);
            } else {
                mysqli_stmt_bind_param($data_stmt, "iii", $accountant_id, $limit, $offset);
            }
            ...
            <div class="content-area">
                ...
                <table class="complaints-table">
                    <thead><tr><th>ID</th><th>Student</th><th>Title</th><th>Priority</th><th>Status</th><th>Date</th><th>Action</th></tr></thead>
                    <tbody>
                        <?php if (mysqli_num_rows($data_result) == 0): ?>
                            <tr><td colspan="7" style="text-align:center;">No complaints found. </div>--> </tr>
                        <?php else: ?>
        ...
        <?php elseif ($active_page == 'view-complaint' && $complaint_id): ?>
            ...
            mysqli_stmt_bind_param($detail_stmt, "ii", $complaint_id, $accountant_id);
            ...
            if (!$complaint) { echo '<div class="alert error">Complaint not found or not assigned to you.</div>'; } else {
                ...
                <div class="content-area">
                    ...
                    <div class="complaint-detail-grid">
                        ...
                        <p><strong>Evidence:</strong> <a href="../<?php echo $complaint['attachment_path']; ?>" target="_blank">Download</a></p>
                        <?php endif; ?>
                    </div>
        ...
        <?php elseif ($active_page == 'profile'): ?>
            ...
            <div class="content-area"><h4>Edit Profile</h4>
                <form method="POST" enctype="multipart/form-data"><input type="hidden" name="update_profile" value="1">
                    <div class="form-group"><label>Full Name</label><input type="text" name="full_name" value="<?php echo htmlspecialchars($accountant_name); ?>" required></div>
                    <div class="form-group"><label>Email (cannot be changed)</label><input type="email" value="<?php echo htmlspecialchars($accountant_email); ?>" disabled></div>
