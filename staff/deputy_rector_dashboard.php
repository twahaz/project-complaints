...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Sidebar (cobalt blue, collapsible) */
        .sidebar {
            width: 280px;
            background: #0047AB;
            color: white;
...
<div class="sidebar" id="sidebar">
    ...
    <div class="sidebar-menu">
        ...
        <a href="?page=all" class="menu-item <?php echo $active_page == 'all' ? 'active' : ''; ?>">
        ...
        <a href="?page=escalated" class="menu-item <?php echo $active_page == 'escalated' ? 'active' : ''; ?>">
            <i class="fas fa-exclamation-triangle"></i><span>Escalated (to Rector)</span>
        </a>
...
<div class="main-content">
    <div class="top-bar">
        <div class="welcome-message">Welcome, <span><?php echo htmlspecialchars($deputy_name); ?></span></div>
    ...
    <div class="dashboard-body">
            ...
            <div class="content-area">
                ...
                <div class="table-responsive">
                    <table class="complaints-table">
                        <thead>
                            <tr><th>ID</th><th>Student</th><th>Title</th><th>Escalated By</th><th>Status</th><th>Date</th><th>Action</th></tr>
                        </thead>
                        <tbody>
                            <?php
        ...
        <?php elseif ($active_page == 'all' || $active_page == 'pending' || $active_page == 'resolved' || $active_page == 'escalated'): ?>
            ...
            if ($active_page == 'pending') {
                ...
                $where = "escalated_to = ? AND status = 'resolved'";
                $use_deputy_id = true;
            } elseif ($active_page == 'escalated') {
                // Complaints escalated to Rector (for the deputy to see as read-only)
                $where = "assigned_to = ? AND status = 'escalated'";
                $use_deputy_id = false;
            } else {
                $where = "escalated_to = ?";
                $use_deputy_id = true;
            ...
            if ($use_deputy_id) {
                ...
                if ($priority_filter) $count_sql .= " AND priority = ?";
                if ($search) $count_sql .= " AND (complaint_number LIKE ? OR student_id IN (SELECT id FROM users WHERE full_name LIKE ?))";
            ...
            <div class="content-area">
                ...
                <div class="table-responsive">
                    <table class="complaints-table">
                        <thead>
                            <tr>
                                <th>ID</th><th>Student</th><th>Title</th><th>Priority</th><th>Escalated By</th><th>Status</th><th>Date</th>
                                <th>Resolution Time</th><th>Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if (mysqli_num_rows($data_result) == 0): ?>
                                <tr><td colspan="9" style="text-align:center;">No complaints found. <?php echo $active_page == 'escalated' ? 'No complaints have been escalated to the Rector yet.' : ''; ?> </td></tr>
                            <?php else: ?>
                                ...
                                <tr>
                                    <td><?php echo htmlspecialchars($row['complaint_number']); ?></td>
                                    <td><?php echo htmlspecialchars($row['student_name']); ?></td>
                                    <td><?php echo htmlspecialchars($row['title']); ?></div> </td>
                                    <td><?php echo ucfirst($row['priority']); ?></div></td>
                                    <td><?php echo htmlspecialchars($row['escalated_by'] . ' (' . ucfirst(str_replace('_', ' ', $row['escalated_by_role'])) . ')'); ?></td>
                                    <td><span class="badge <?php echo $status_class; ?>"><?php echo ucfirst($row['status']); ?></span></td>
                                    <td><?php echo date('d/m/Y', strtotime($row['created_at'])); ?></div> </td>
                                    <td><?php echo $resolution_time; ?></div></td>
                                    <td><a href="?page=view-complaint&id=<?php echo $row['id']; ?>" class="btn-sm">View</a></td>
                                </tr>
                                <?php endwhile; ?>
                            <?php endif; ?>
        ...
        <?php elseif ($active_page == 'view-complaint' && $complaint_id): ?>
            <?php
            // First check if complaint belongs to deputy (escalated_to) OR is already escalated to rector
            ...
            if (!$complaint) { echo '<div class="alert error">Complaint not found or not accessible to you.</div>'; } else {
                ...
                <div class="content-area">
                    <?php if ($is_readonly): ?>
                        <div class="alert" style="background:#fff3cd; color:#856404; border-left-color:#ffc107;">
                            <i class="fas fa-lock"></i> This complaint has been escalated to the Rector and is now <strong>read‑only</strong>. You cannot reply or change its status.
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="complaint-detail-grid">
                        ...
                        <p><strong>Escalated By:</strong> <?php echo htmlspecialchars($complaint['escalated_by']); ?> (<?php echo ucfirst(str_replace('_', ' ', $complaint['escalated_by_role'])); ?>)</p>
                        ...
                        <div class="reply-section">
                            ...
                            <form method="POST">
                                ...
                                <div class="form-group">
                                    <label><input type="checkbox" name="escalate_to_rector" value="1"> Escalate to Rector</label>
                                </div>
                    ...
                    <?php else: ?>
                        <div class="alert" style="background:#e9ecef; color:#495057;">
        ...
        <?php elseif ($active_page == 'profile'): ?>
            ...
            <div class="content-area"><h4>Edit Profile</h4>
                <form method="POST" enctype="multipart/form-data"><input type="hidden" name="update_profile" value="1">
                    <div class="form-group"><label>Full Name</label><input type="text" name="full_name" value="<?php echo htmlspecialchars($deputy_name); ?>" required></div>
                    <div class="form-group"><label>Email (cannot be changed)</label><input type="email" value="<?php echo htmlspecialchars($deputy_email); ?>" disabled></div>
