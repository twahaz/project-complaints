...
$dean_name = $_SESSION['full_name'];
$dean_email = $_SESSION['email'];

// Fetch department name (Dean of Students)
...
mysqli_stmt_bind_param($dept_stmt, "i", $dean_id);
mysqli_stmt_execute($dept_stmt);
$dept_result = mysqli_stmt_get_result($dept_stmt);
$dept_row = mysqli_fetch_assoc($dept_result);
$dept_name = $dept_row['name'] ?? 'Dean of Students';
mysqli_stmt_close($dept_stmt);

// Categories that belong to dean
$dean_categories = ['Hostel', 'Infrastructure', 'Service', 'Gender issue'];
$category_in_list = "'" . implode("','", $dean_categories) . "'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reply_complaint'])) {
    ...
    // Check if complaint is escalated (read‑only)
    $check_sql = "SELECT status FROM complaints WHERE id = ? AND assigned_to = ?";
    $check_stmt = mysqli_prepare($conn, $check_sql);
    mysqli_stmt_bind_param($check_stmt, "ii", $complaint_id, $dean_id);
    ...
    if ($check_row && $check_row['status'] === 'escalated') {
        ...
        header("Location: dean_dashboard.php?page=view-complaint&id=" . $complaint_id);
        exit();
    }
    ...
    if (!empty($message)) {
        ...
        mysqli_stmt_bind_param($insert_stmt, "iis", $complaint_id, $dean_id, $message);
        ...
        if ($new_status === 'escalated') {
            ...
            if ($deputy_id) {
                // Keep assigned_to unchanged, set escalated_to = deputy_id
        ...
        if ($new_status === 'escalated') {
            header("Location: dean_dashboard.php?page=escalated");
        } else {
            header("Location: dean_dashboard.php?page=complaints");
        }
        exit();
    } else {
...
// Handle profile update (unchanged from your original code)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($file['type'], $allowed_types)) {
            ...
            $new_filename = 'dean_' . $dean_id . '_' . time() . '.' . $extension;
    ...
    if (empty($errors)) {
        ...
        $update_stmt = mysqli_prepare($conn, $update_sql);
        mysqli_stmt_bind_param($update_stmt, "sssi", $new_full_name, $new_phone, $new_profile_picture, $dean_id);
        mysqli_stmt_execute($update_stmt);
        mysqli_stmt_close($update_stmt);
        ...
        $_SESSION['flash_type'] = "success";
        header("Location: dean_dashboard.php?page=profile");
        exit();
    } else {
...
<head>
    ...
    <title>Dean Dashboard - IAA CFMS</title>
    <link rel="icon" type="image/png" href="../images/logo.png">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
    <style>
        /* ========== FULL CSS (identical to the HOD dashboard) ========== */
        * {
            margin: 0;
            padding: 0;
...
<div class="sidebar" id="sidebar">
    <div class="sidebar-header">
        ...
        <div class="row-tagline">
            <span class="tagline">Dean Portal</span>
...
<div class="main-content">
    <div class="top-bar">
        <div class="welcome-message">Welcome, <span><?php echo htmlspecialchars($dean_name); ?></span><br><small style="font-size:0.8rem;"><?php echo htmlspecialchars($dept_name); ?></small></div>
        <div class="profile-info">
            <div class="profile-pic">
                ...
                <?php else: ?>
                    <?php echo strtoupper(substr($dean_name, 0, 1)); ?>
                <?php endif; ?>
            </div>
            <div class="profile-details">
                <div class="name"><?php echo htmlspecialchars($dean_name); ?></div>
                <div class="reg">Dean</div>
            </div>
        </div>
    </div>
    ...
    <div class="dashboard-body">
        ...
        <?php if ($active_page == 'dashboard'): ?>
            ...
            $stats_sql = "SELECT 
                ...
                WHERE c.assigned_to = ? AND cat.name IN ($category_in_list)";
            $stats_stmt = mysqli_prepare($conn, $stats_sql);
            mysqli_stmt_bind_param($stats_stmt, "i", $dean_id);
            ...
            <div class="content-area">
                ...
                mysqli_stmt_bind_param($recent_stmt, "i", $dean_id);
        ...
        <?php elseif ($active_page == 'complaints' || $active_page == 'pending' || $active_page == 'resolved' || $active_page == 'escalated'): ?>
            ...
            $where = "c.assigned_to = ? AND cat.name IN ($category_in_list) $status_filter";
            ...
            if ($priority_filter && $search) {
                $search_param = "%$search%";
                mysqli_stmt_bind_param($count_stmt, "iss", $dean_id, $priority_filter, $search_param);
            } elseif ($priority_filter) {
                mysqli_stmt_bind_param($count_stmt, "is", $dean_id, $priority_filter);
            } elseif ($search) {
                $search_param = "%$search%";
                mysqli_stmt_bind_param($count_stmt, "is", $dean_id, $search_param);
            } else {
                mysqli_stmt_bind_param($count_stmt, "i", $dean_id);
            }
            ...
            if ($priority_filter && $search) {
                $search_param = "%$search%";
                mysqli_stmt_bind_param($data_stmt, "issii", $dean_id, $priority_filter, $search_param, $limit, $offset);
            } elseif ($priority_filter) {
                mysqli_stmt_bind_param($data_stmt, "isii", $dean_id, $priority_filter, $limit, $offset);
            } elseif ($search) {
                $search_param = "%$search%";
                mysqli_stmt_bind_param($data_stmt, "isii", $dean_id, $search_param, $limit, $offset);
            } else {
                mysqli_stmt_bind_param($data_stmt, "iii", $dean_id, $limit, $offset);
            }
        ...
        <?php elseif ($active_page == 'view-complaint' && $complaint_id): ?>
            ...
            $detail_sql = "SELECT c.*, u.full_name as student_name, u.reg_number, u.email as student_email 
                          ...
                          JOIN users u ON c.student_id = u.id 
                          JOIN categories cat ON c.category_id = cat.id
                          WHERE c.id = ? AND c.assigned_to = ? AND cat.name IN ($category_in_list)";
            $detail_stmt = mysqli_prepare($conn, $detail_sql);
            mysqli_stmt_bind_param($detail_stmt, "ii", $complaint_id, $dean_id);
                ...
                <div class="content-area">
                    <?php if ($is_escalated): ?>
                        <div class="alert" style="background:#fff3cd; color:#856404; border-left-color:#ffc107;">
                            <i class="fas fa-lock"></i> This complaint has been escalated and is now <strong>read‑only</strong>. You cannot reply or change its status.
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="complaint-detail-grid">
                        <p><strong><i class="fas fa-user-graduate"></i> Student:</strong> <?php echo htmlspecialchars($complaint['student_name']); ?> (<?php echo $complaint['reg_number']; ?>)</p>
                        <p><strong><i class="fas fa-heading"></i> Title:</strong> <?php echo htmlspecialchars($complaint['title']); ?></p>
                        <p><strong><i class="fas fa-map-marker-alt"></i> Location:</strong> <?php echo htmlspecialchars($complaint['location'] ?: 'Not provided'); ?></p>
                        <p><strong><i class="fas fa-calendar-alt"></i> Incident Date:</strong> <?php echo date('d/m/Y', strtotime($complaint['incident_date'])); ?></p>
                        <p><strong><i class="fas fa-flag"></i> Priority:</strong> <?php echo ucfirst($complaint['priority']); ?></p>
                        <p><strong><i class="fas fa-tag"></i> Status:</strong> <span class="badge <?php 
                        ...
                        ?>"><?php echo ucfirst($complaint['status']); ?></span></p>
                        <?php if ($complaint['attachment_path']): ?>
                        <p><strong><i class="fas fa-paperclip"></i> Evidence:</strong> <a href="../<?php echo $complaint['attachment_path']; ?>" target="_blank" class="download-link"><i class="fas fa-download"></i> Download</a></p>
                        <?php endif; ?>
                    </div>
                    ...
                    <h5><i class="fas fa-comments"></i> Conversation</h5>
                    <div class="conversation">
                        <?php while ($resp = mysqli_fetch_assoc($resp_result)): ?>
                            <div class="message <?php echo ($resp['role'] == 'student') ? 'student-message' : 'staff-message'; ?>">
                                <div class="message-header">
                                    <strong><?php echo htmlspecialchars($resp['full_name']); ?> (<?php echo ucfirst($resp['role']); ?>)</strong>
                        ...
                        <?php if ($resp_stmt && mysqli_num_rows($resp_result) == 0): ?>
                            <p>No messages yet. Use the form below to reply.</p>
                        <?php endif; ?>
                    </div>
                    
                    <?php if (!$is_escalated): ?>
                        <div class="reply-section">
                            <h5><i class="fas fa-reply"></i> Reply to Student</h5>
                            <form method="POST">
                                <input type="hidden" name="complaint_id" value="<?php echo $complaint_id; ?>">
                                <div class="form-group">
                                    <label>Your Message</label>
                                    <textarea name="message" rows="4" required placeholder="Write your reply here..."></textarea>
                                </div>
                                <div class="form-group">
                                    ...
                                    <select name="status">
                                        ...
                                        <option value="escalated" <?php echo $complaint['status']=='escalated' ? 'selected' : ''; ?>>Escalate to Deputy Rector</option>
                                    </select>
                                </div>
                        ...
                        <div class="status-section">
                            <h5><i class="fas fa-edit"></i> Change Status Only</h5>
                            <form method="POST">
                                <input type="hidden" name="complaint_id" value="<?php echo $complaint_id; ?>">
                                <div class="form-group">
        ...
        <?php elseif ($active_page == 'profile'): ?>
            ...
            mysqli_stmt_bind_param($prof_stmt, "i", $dean_id);
            ...
            <div class="content-area">
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group"><label>Full Name</label><input type="text" name="full_name" value="<?php echo htmlspecialchars($dean_name); ?>" required></div>
                    <div class="form-group"><label>Email (cannot be changed)</label><input type="email" value="<?php echo htmlspecialchars($dean_email); ?>" disabled></div>
...
<div id="logoutModal" class="modal-overlay">
    <div class="modal-container">
        ...
        <div class="modal-buttons"><button class="modal-btn confirm" id="confirmLogout">Yes, Logout</button><button class="modal-btn cancel" id="cancelLogout">No, Cancel</button></div>
    </div>
</div>
