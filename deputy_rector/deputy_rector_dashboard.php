...
// Fetch rector ID once
$rector_sql = "SELECT id FROM users WHERE role = 'rector' LIMIT 1";
$rector_stmt = mysqli_prepare($conn, $rector_sql);
mysqli_stmt_execute($rector_stmt);
$rector_result = mysqli_stmt_get_result($rector_stmt);
$rector_row = mysqli_fetch_assoc($rector_result);
$rector_id = $rector_row['id'] ?? null;
mysqli_stmt_close($rector_stmt);

// Handle reply to complaint (including escalation to Rector)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reply_complaint'])) {
    $complaint_id = intval($_POST['complaint_id']);
    $message = trim($_POST['message']);
    $new_status = $_POST['status'] ?? 'in_progress';
    $escalate_to_rector = isset($_POST['escalate_to_rector']) && $_POST['escalate_to_rector'] == '1';
    
    if (!empty($message)) {
        $insert_sql = "INSERT INTO responses (complaint_id, user_id, message) VALUES (?, ?, ?)";
        $insert_stmt = mysqli_prepare($conn, $insert_sql);
        mysqli_stmt_bind_param($insert_stmt, "iis", $complaint_id, $deputy_id, $message);
        mysqli_stmt_execute($insert_stmt);
        mysqli_stmt_close($insert_stmt);
        
        if ($escalate_to_rector) {
            if ($rector_id) {
                $update_sql = "UPDATE complaints SET status = 'escalated', assigned_to = ?, escalated_to = NULL, updated_at = NOW() WHERE id = ?";
                $update_stmt = mysqli_prepare($conn, $update_sql);
                mysqli_stmt_bind_param($update_stmt, "ii", $rector_id, $complaint_id);
                mysqli_stmt_execute($update_stmt);
                mysqli_stmt_close($update_stmt);
                $_SESSION['flash_message'] = "Reply sent and complaint escalated to Rector.";
            } else {
                $_SESSION['flash_message'] = "Could not escalate: Rector not found.";
                $_SESSION['flash_type'] = "error";
                header("Location: deputy_rector_dashboard.php?page=view-complaint&id=" . $complaint_id);
                exit();
            }
        } else {
        ...
        header("Location: deputy_rector_dashboard.php?page=all");
        exit();
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    if ($escalate_to_rector) {
        if ($rector_id) {
            ...
            mysqli_stmt_execute($update_stmt);
            mysqli_stmt_close($update_stmt);
            $_SESSION['flash_message'] = "Complaint escalated to Rector.";
        } else {
            $_SESSION['flash_message'] = "Rector not found.";
            $_SESSION['flash_type'] = "error";
        }
    } else {
        $update_sql = "UPDATE complaints SET status = ?, updated_at = NOW() WHERE id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($file['type'], $allowed_types)) {
            ...
            $new_filename = 'deputy_' . $deputy_id . '_' . time() . '.' . $extension;
    ...
    if (empty($errors)) {
        ...
        $update_stmt = mysqli_prepare($conn, $update_sql);
        mysqli_stmt_bind_param($update_stmt, "sssi", $new_full_name, $new_phone, $new_profile_picture, $deputy_id);
        mysqli_stmt_execute($update_stmt);
        mysqli_stmt_close($update_stmt);
        ...
        $_SESSION['flash_type'] = "success";
        header("Location: deputy_rector_dashboard.php?page=profile");
        exit();
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_password'])) {
    ...
    if (empty($errors)) {
        ...
        mysqli_stmt_bind_param($pass_stmt, "si", $hashed, $deputy_id);
        mysqli_stmt_execute($pass_stmt);
        mysqli_stmt_close($pass_stmt);
        $_SESSION['flash_message'] = "Password changed successfully!";
        $_SESSION['flash_type'] = "success";
        header("Location: deputy_rector_dashboard.php?page=change-password");
        exit();
    } else {
        $_SESSION['flash_message'] = implode("<br>", $errors);
        $_SESSION['flash_type'] = "error";
...
mysqli_stmt_bind_param($prof_stmt, "i", $deputy_id);
...
$stats_sql = "SELECT 
    ...
    FROM complaints WHERE escalated_to = ?";
$stats_stmt = mysqli_prepare($conn, $stats_sql);
mysqli_stmt_bind_param($stats_stmt, "i", $deputy_id);
...
$recent_sql = "SELECT c.id, c.complaint_number, c.title, c.status, c.created_at, u.full_name as student_name,
                      staff.full_name as escalated_by, staff.role as escalated_by_role
              FROM complaints c
              JOIN users u ON c.student_id = u.id
              JOIN users staff ON c.assigned_to = staff.id
              WHERE c.escalated_to = ?
              ORDER BY c.created_at DESC LIMIT 5";
$recent_stmt = mysqli_prepare($conn, $recent_sql);
mysqli_stmt_bind_param($recent_stmt, "i", $deputy_id);
...
if ($table_check && mysqli_num_rows($table_check) > 0) {
    ...
    if ($ann_stmt) {
        ...
        mysqli_stmt_bind_param($ann_stmt, "ii", $dept_id, $deputy_id);
...
<head>
    ...
    <title>Deputy Rector Dashboard - IAA CFMS</title>
    <link rel="icon" type="image/png" href="../images/logo.png">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">
    <style>
        /* ============================================
           UI SANA NA DASHBOARD ZINGINE
           ============================================ */
...
<div class="sidebar" id="sidebar">
    ...
    <div class="sidebar-menu">
        <a href="deputy_rector_dashboard.php?page=dashboard" class="menu-item <?php echo $active_page == 'dashboard' ? 'active' : ''; ?>">
            <i class="fas fa-tachometer-alt"></i><span>Dashboard</span>
        </a>
        <a href="deputy_rector_dashboard.php?page=all" class="menu-item <?php echo $active_page == 'all' ? 'active' : ''; ?>">
            <i class="fas fa-list"></i><span>All Complaints</span>
        </a>
        <a href="deputy_rector_dashboard.php?page=pending" class="menu-item <?php echo $active_page == 'pending' ? 'active' : ''; ?>">
            <i class="fas fa-clock"></i><span>Pending</span>
        </a>
        <a href="deputy_rector_dashboard.php?page=resolved" class="menu-item <?php echo $active_page == 'resolved' ? 'active' : ''; ?>">
            <i class="fas fa-check-circle"></i><span>Resolved</span>
        </a>
        <a href="deputy_rector_dashboard.php?page=escalated" class="menu-item <?php echo $active_page == 'escalated' ? 'active' : ''; ?>">
            <i class="fas fa-exclamation-triangle"></i><span>Escalated</span>
        </a>
        <a href="announcements.php" class="menu-item">
            <i class="fas fa-bullhorn"></i><span>Announcements</span>
        </a>
        <a href="deputy_rector_dashboard.php?page=profile" class="menu-item <?php echo $active_page == 'profile' ? 'active' : ''; ?>">
            <i class="fas fa-user-circle"></i><span>Profile</span>
        </a>
        <a href="deputy_rector_dashboard.php?page=change-password" class="menu-item <?php echo $active_page == 'change-password' ? 'active' : ''; ?>">
...
<div class="main-content">
    ...
    <div class="top-bar">
        <div>
            <div style="font-size: 0.9rem; font-weight: 600; color: #0a2a5e;">
                <i class="fas fa-user-tie" style="color: #1a56db;"></i> Welcome, <?php echo htmlspecialchars($deputy_name); ?>
            </div>
    ...
    <div class="dashboard-body">
                ...
                <div class="content-area">
                    ...
                    <div class="table-responsive">
                        <table class="complaints-table">
                            <thead>
                                <tr><th>Complaint #</th><th>Student</th><th>Title</th><th>Escalated By</th><th>Status</th><th>Date</th></tr>
                            </thead>
                            <tbody>
                                <?php if (mysqli_num_rows($recent_result) == 0): ?>
                                    <tr><td colspan="6" style="text-align:center; padding:30px; color:#8ba0bc;">No complaints escalated to you yet.</td></tr>
                                <?php else: ?>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td><?php echo htmlspecialchars($row['escalated_by'] . ' (' . ucfirst(str_replace('_', ' ', $row['escalated_by_role'])) . ')'); ?></td>
                    ...
                    <div style="margin-top: 15px; text-align: center;">
                        <a href="deputy_rector_dashboard.php?page=all" class="btn-sm" style="background: #6b85a0;">View All Complaints <i class="fas fa-arrow-right"></i></a>
                    </div>
                </div>
        ...
        <?php elseif ($active_page == 'all' || $active_page == 'pending' || $active_page == 'resolved' || $active_page == 'escalated'): ?>
            <?php
            if ($active_page == 'pending') {
                $where = "c.escalated_to = ? AND c.status = 'pending'";
                $use_deputy_id = true;
            } elseif ($active_page == 'resolved') {
                $where = "c.escalated_to = ? AND c.status = 'resolved'";
                $use_deputy_id = true;
            } elseif ($active_page == 'escalated') {
                $where = "c.assigned_to = ? AND c.status = 'escalated'";
                $use_deputy_id = false;
            } else {
                $where = "c.escalated_to = ?";
                $use_deputy_id = true;
            ...
            // Build COUNT query
            if ($use_deputy_id) {
                $count_sql = "SELECT COUNT(*) as total FROM complaints c JOIN users u ON c.student_id = u.id WHERE $where";
                if ($priority_filter) $count_sql .= " AND c.priority = ?";
                if ($search) $count_sql .= " AND (c.complaint_number LIKE ? OR u.full_name LIKE ?)";
                $count_stmt = mysqli_prepare($conn, $count_sql);
                if ($priority_filter && $search) {
                    $search_param = "%$search%";
                    mysqli_stmt_bind_param($count_stmt, "iss", $deputy_id, $priority_filter, $search_param);
                } elseif ($priority_filter) {
                    mysqli_stmt_bind_param($count_stmt, "is", $deputy_id, $priority_filter);
                } elseif ($search) {
                    $search_param = "%$search%";
                    mysqli_stmt_bind_param($count_stmt, "is", $deputy_id, $search_param);
                } else {
                    mysqli_stmt_bind_param($count_stmt, "i", $deputy_id);
                }
            } else {
                $count_sql = "SELECT COUNT(*) as total FROM complaints c JOIN users u ON c.student_id = u.id WHERE $where";
                if ($priority_filter) $count_sql .= " AND c.priority = ?";
            ...
            // Build SELECT query
            if ($use_deputy_id) {
                $data_sql = "SELECT c.id, c.complaint_number, c.title, c.status, c.created_at, c.updated_at, u.full_name as student_name, c.priority,
                                    staff.full_name as escalated_by, staff.role as escalated_by_role
                            FROM complaints c
                            JOIN users u ON c.student_id = u.id
                            JOIN users staff ON c.assigned_to = staff.id
                ...
                if ($priority_filter && $search) {
                    $search_param = "%$search%";
                    mysqli_stmt_bind_param($data_stmt, "issii", $deputy_id, $priority_filter, $search_param, $limit, $offset);
                } elseif ($priority_filter) {
                    mysqli_stmt_bind_param($data_stmt, "isii", $deputy_id, $priority_filter, $limit, $offset);
                } elseif ($search) {
                    $search_param = "%$search%";
                    mysqli_stmt_bind_param($data_stmt, "isii", $deputy_id, $search_param, $limit, $offset);
                } else {
                    mysqli_stmt_bind_param($data_stmt, "iii", $deputy_id, $limit, $offset);
                }
            } else {
                $data_sql = "SELECT c.id, c.complaint_number, c.title, c.status, c.created_at, c.updated_at, u.full_name as student_name, c.priority,
                                    staff.full_name as escalated_by, staff.role as escalated_by_role
            ...
            <div class="content-area">
                ...
                <div class="table-responsive">
                    <table class="complaints-table">
                        <thead>
                            <tr><th>Complaint #</th><th>Student</th><th>Title</th><th>Priority</th><th>Escalated By</th><th>Status</th><th>Date</th><th>Action</th></tr>
                        </thead>
                        <tbody>
                            ...
                            <?php else: ?>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td><span class="badge <?php echo $status_class; ?>"><?php echo ucfirst($row['status']); ?></span></td>
                                        <td><?php echo date('d/m/Y', strtotime($row['created_at'])); ?></td>
                                        <td><a href="deputy_rector_dashboard.php?page=view-complaint&id=<?php echo $row['id']; ?>" class="btn-sm">View</a></td>
                                    </tr>
                                <?php endwhile; ?>
                            <?php endif; ?>
                        </tbody>
                ...
                <div class="pagination">
                    <?php if ($page > 1): ?>
                        <a href="deputy_rector_dashboard.php?page=<?php echo $active_page; ?>&search=<?php echo urlencode($search); ?>&priority=<?php echo $priority_filter; ?>&sort=<?php echo $sort; ?>&p=<?php echo $page-1; ?>"><i class="fas fa-chevron-left"></i></a>
                    <?php endif; ?>
                    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                        <?php if ($i == $page): ?>
                            <span class="current"><?php echo $i; ?></span>
                        <?php else: ?>
                            <a href="deputy_rector_dashboard.php?page=<?php echo $active_page; ?>&search=<?php echo urlencode($search); ?>&priority=<?php echo $priority_filter; ?>&sort=<?php echo $sort; ?>&p=<?php echo $i; ?>"><?php echo $i; ?></a>
                        <?php endif; ?>
                    <?php endfor; ?>
                    <?php if ($page < $total_pages): ?>
                        <a href="deputy_rector_dashboard.php?page=<?php echo $active_page; ?>&search=<?php echo urlencode($search); ?>&priority=<?php echo $priority_filter; ?>&sort=<?php echo $sort; ?>&p=<?php echo $page+1; ?>"><i class="fas fa-chevron-right"></i></a>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
            </div>
        ...
        <?php elseif ($active_page == 'view-complaint' && $complaint_id): ?>
            ...
            $detail_sql = "SELECT c.*, u.full_name as student_name, u.reg_number, 
                          ...
                          JOIN users u ON c.student_id = u.id
                          JOIN users staff ON c.assigned_to = staff.id
                          WHERE c.id = ? AND (c.escalated_to = ? OR (c.assigned_to = ? AND c.status = 'escalated'))";
            $detail_stmt = mysqli_prepare($conn, $detail_sql);
            mysqli_stmt_bind_param($detail_stmt, "iii", $complaint_id, $deputy_id, $rector_id);
            ...
            if (!$complaint) { 
                echo '<div class="content-area"><p style="color:#991b1b;">Complaint not found or not accessible to you.</p></div>';
            } else {
                $is_readonly = ($complaint['assigned_to'] == $rector_id && $complaint['status'] == 'escalated');
                ...
                <div class="content-area">
                    <?php if ($is_readonly): ?>
                        <div style="background:#fef3c7; color:#856404; border-left:4px solid #f59e0b; padding:12px 16px; border-radius:12px; margin-bottom:20px; display:flex; align-items:center; gap:10px;">
                            <i class="fas fa-lock"></i> This complaint has been escalated to the Rector and is now <strong>read‑only</strong>.
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="complaint-detail-grid">
                        ...
                        <div class="detail-item">
                            ...
                            <span class="value"><?php echo htmlspecialchars($complaint['escalated_by']); ?> (<?php echo ucfirst(str_replace('_', ' ', $complaint['escalated_by_role'])); ?>)</span>
                        </div>
                        <div class="detail-item">
                    ...
                    <?php if (!$is_readonly): ?>
                        ...
                        <div class="reply-section">
                            ...
                            <form method="POST">
                                ...
                                <div class="form-group" style="display:flex; align-items:center; gap:10px;">
                                    <input type="checkbox" name="escalate_to_rector" value="1" id="escalate_reply">
                                    <label for="escalate_reply" style="margin-bottom:0; cursor:pointer;">Escalate to Rector</label>
                                </div>
                        ...
                        <div class="status-section">
                            ...
                            <form method="POST">
                                ...
                                <div class="form-group" style="display:flex; align-items:center; gap:10px;">
                                    <input type="checkbox" name="escalate_to_rector" value="1" id="escalate_status">
                                    <label for="escalate_status" style="margin-bottom:0; cursor:pointer;">Escalate to Rector</label>
                                </div>
                    ...
                    <?php else: ?>
                        <div style="background:#e9ecef; color:#495057; border-left:4px solid #6c757d; padding:12px 16px; border-radius:12px; margin-top:20px; display:flex; align-items:center; gap:10px;">
                            <i class="fas fa-eye"></i> This complaint is read‑only because it has been escalated to the Rector.
                        </div>
                    <?php endif; ?>
                </div>
            <?php } ?>
            ...
            <div class="content-area">
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="full_name" value="<?php echo htmlspecialchars($deputy_name); ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Email (cannot be changed)</label>
                        <input type="email" value="<?php echo htmlspecialchars($deputy_email); ?>" disabled style="background:#f0f4f9; cursor:not-allowed;">
                    </div>
                    <div class="form-group">
