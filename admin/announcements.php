...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_announcement'])) {
    ...
    if (empty($errors)) {
        ...
        if ($insert_stmt) {
            mysqli_stmt_bind_param($insert_stmt, "sssiiss", $title, $message, $target_type, $target_id, $admin_id, $is_active, $expiry_date);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_announcement'])) {
    ...
    if (empty($errors)) {
        $update_sql = "UPDATE announcements SET title = ?, message = ?, target_type = ?, target_id = ?, is_active = ? WHERE id = ?";
        $update_stmt = mysqli_prepare($conn, $update_sql);
        if ($update_stmt) {
            mysqli_stmt_bind_param($update_stmt, "sssiii", $title, $message, $target_type, $target_id, $is_active, $announcement_id);
        ...
        $log_stmt = mysqli_prepare($conn, $log_sql);
        $description = "Updated announcement: $title (ID: $announcement_id)";
        $ip = $_SERVER['REMOTE_ADDR'];
        mysqli_stmt_bind_param($log_stmt, "iss", $admin_id, $description, $ip);
        mysqli_stmt_execute($log_stmt);
        mysqli_stmt_close($log_stmt);
...
if (isset($_GET['delete'])) {
    ...
    $title_stmt = mysqli_prepare($conn, $title_sql);
    ...
    $ann_title = $title_row['title'] ?? 'Unknown';
    ...
    $log_stmt = mysqli_prepare($conn, $log_sql);
    $description = "Deleted announcement: $ann_title (ID: $announcement_id)";
    $ip = $_SERVER['REMOTE_ADDR'];
    mysqli_stmt_bind_param($log_stmt, "iss", $admin_id, $description, $ip);
    mysqli_stmt_execute($log_stmt);
    mysqli_stmt_close($log_stmt);
                            ...
                            <div class="card-footer">
                                <div class="meta">
                                    <span>
                                        ...
                                        From: <span class="badge <?php echo $creator_class; ?>" style="font-size:0.6rem; padding:2px 10px;"><?php echo $creator_label; ?></span>
                                ...
                                <div class="actions">
                                    <a href="announcements.php?action=edit&id=<?php echo $ann['id']; ?>" class="btn-sm warning">
...
<div id="deleteModal" class="modal-overlay">
    <div class="modal-container">
        ...
        <div class="modal-buttons">
            <button class="modal-btn confirm" id="confirmDeleteBtn"><i class="fas fa-trash"></i> Yes, Delete</button>
            <button class="modal-btn cancel" id="cancelDeleteBtn"><i class="fas fa-times"></i> Cancel</button>
        </div>
    </div>
</div>
...
<script>
    ...
    document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
        if (deleteId) {
            window.location.href = 'announcements.php?delete=' + deleteId;
        }
    });
