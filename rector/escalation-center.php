...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['reply_complaint'])) {
        ...
        if (!empty($message)) {
            ...
            // Update complaint status - remove updated_by
            ...
            header("Location: escalation-center.php?action=view&id=" . $complaint_id);
            exit();
        } else {
    ...
    if (isset($_POST['update_status'])) {
        ...
        $_SESSION['flash_type'] = "success";
        header("Location: escalation-center.php");
        exit();
    }
}
...
// ========== GET ESCALATION STATS - FOR INTERNAL USE ONLY ==========
$stats_sql = "SELECT 
    COUNT(*) as total,
    SUM(CASE WHEN status = 'pending' THEN 1 ELSE 0 END) as pending,
    SUM(CASE WHEN status = 'in_progress' THEN 1 ELSE 0 END) as in_progress,
    SUM(CASE WHEN status = 'resolved' THEN 1 ELSE 0 END) as resolved
    FROM complaints WHERE assigned_to = ? AND status = 'escalated'";
...
$where = "c.assigned_to = $rector_id AND c.status = 'escalated'";
if ($priority_filter) $where .= " AND c.priority = '" . mysqli_real_escape_string($conn, $priority_filter) . "'";
if ($search) {
    $search_escaped = mysqli_real_escape_string($conn, $search);
    $where .= " AND (c.complaint_number LIKE '%$search_escaped%' OR u.full_name LIKE '%$search_escaped%')";
}
...
// Count total escalated complaints
...
// Get escalated complaints data - using escalated_by column
$data_sql = "SELECT c.*, u.full_name as student_name, u.reg_number,
                    esc.full_name as escalated_by_name, esc.role as escalated_by_role
             FROM complaints c 
             JOIN users u ON c.student_id = u.id 
             LEFT JOIN users esc ON c.escalated_by = esc.id
             WHERE $where
             ORDER BY $order_by
...
<head>
    ...
    <title>Escalation Center - Rector Panel</title>
...
<div class="sidebar" id="sidebar">
    ...
    <div class="sidebar-menu">
        ...
        <a href="escalation-center.php" class="menu-item active">
    ...
    <div class="dashboard-body">
        ...
        <?php if ($action === 'view' && $complaint_id): ?>
            ...
            $detail_sql = "SELECT c.*, u.full_name as student_name, u.reg_number,
                          ...
                          JOIN users u ON c.student_id = u.id 
                          LEFT JOIN users esc ON c.escalated_by = esc.id
                          WHERE c.id = ? AND c.assigned_to = ? AND c.status = 'escalated'";
            ...
            if (!$complaint) { 
                echo '<div class="content-area"><p style="color:#991b1b;">Complaint not found or not escalated to you.</p></div>';
            } else {
                ...
                <div class="content-area">
                    ...
                    <div style="margin-bottom: 16px;">
                        <a href="escalation-center.php" class="btn-back">
                            <i class="fas fa-arrow-left"></i>
                            <span>Back to Escalation Center</span>
                        </a>
                    </div>

                    <div class="sla-card">
                        <div class="sla-title"><i class="fas fa-exclamation-triangle"></i> Escalated Complaint - SLA Monitoring</div>
                        <div class="sla-stats">
                            <div class="sla-item">
                            ...
                            <div class="sla-item">
                                <div class="sla-label"><i class="fas fa-user-tie"></i> Escalated By</div>
                                <div class="sla-value">
                                    <?php 
                                    if ($complaint['escalated_by_name']) {
                                        echo htmlspecialchars($complaint['escalated_by_name']) . ' (' . ucfirst(str_replace('_', ' ', $complaint['escalated_by_role'] ?? '')) . ')';
                                    } else {
                                        echo 'Staff Member';
                                    }
                                    ?>
                    ...
                    <!-- Reply Section - Only if not resolved -->
                        ...
                        <div class="reply-section">
                            ...
                            <form method="POST">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="message" rows="4" required placeholder="Write your response to the student..."></textarea>
                                </div>
                                <div class="form-group">
                    ...
                    <?php else: ?>
                        <div style="background:#d1fae5; padding:16px 20px; border-radius:12px; margin-top:20px; text-align:center; color:#065f46;">
                            <i class="fas fa-check-circle" style="display:block; font-size:1.5rem; margin-bottom:8px;"></i>
                            This escalated complaint has been resolved.
                        </div>
                    <?php endif; ?>
                </div>
            <?php } ?>
        ...
        <?php else: ?>
            <!-- ========== ESCALATION CENTER LIST VIEW (NO SUMMARY CARDS) ========== -->
            <div class="content-area">
                <h4><i class="fas fa-arrow-up" style="color:#dc2626;"></i> Escalation Center</h4>
                <p style="color:#6b85a0; margin-bottom:16px; font-size:0.85rem;">
                    Complaints that have been escalated to you for review and action.
                </p>
                    ...
                    <table class="complaints-table">
                        ...
                        <tbody>
                            <?php if (mysqli_num_rows($data_result) == 0): ?>
                                <tr><td colspan="8" style="text-align:center; padding:30px; color:#8ba0bc;">
                            ...
                            <?php else: while ($row = mysqli_fetch_assoc($data_result)):
                                ...
                                $escalated_by = $row['escalated_by_name'] 
                                    ? htmlspecialchars($row['escalated_by_name']) . ' (' . ucfirst(str_replace('_', ' ', $row['escalated_by_role'] ?? '')) . ')' 
                                    : 'Staff Member';
                            ?>
                            <tr>
                                ...
                                <td style="font-size:0.7rem;"><?php echo $escalated_by; ?></td>
                                <td><?php echo date('d/m/Y', strtotime($row['created_at'])); ?></td>
                                <td><a href="escalation-center.php?action=view&id=<?php echo $row['id']; ?>" class="btn-sm">View</a></td>
                            </tr>
                            <?php endwhile; endif; ?>
                        </tbody>
                    </table>
                ...
                <div class="pagination">
                    <?php if ($current_list_page > 1): ?>
                        <a href="escalation-center.php?search=<?php echo urlencode($search); ?>&priority=<?php echo $priority_filter; ?>&sort=<?php echo $sort; ?>&p=<?php echo $current_list_page-1; ?>"><i class="fas fa-chevron-left"></i></a>
                    <?php endif; ?>
                    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                        <?php if ($i == $current_list_page): ?>
                            <span class="current"><?php echo $i; ?></span>
                        <?php else: ?>
                            <a href="escalation-center.php?search=<?php echo urlencode($search); ?>&priority=<?php echo $priority_filter; ?>&sort=<?php echo $sort; ?>&p=<?php echo $i; ?>"><?php echo $i; ?></a>
                        <?php endif; ?>
                    <?php endfor; ?>
                    <?php if ($current_list_page < $total_pages): ?>
                        <a href="escalation-center.php?search=<?php echo urlencode($search); ?>&priority=<?php echo $priority_filter; ?>&sort=<?php echo $sort; ?>&p=<?php echo $current_list_page+1; ?>"><i class="fas fa-chevron-right"></i></a>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
            </div>
