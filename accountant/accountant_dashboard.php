...
$accountant_name = $_SESSION['full_name'];
$accountant_email = $_SESSION['email'];

// Categories that belong to accountant
$accountant_categories = ['Accountant', 'Fees'];
$category_in_list = "'" . implode("','", $accountant_categories) . "'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reply_complaint'])) {
    ...
    if (!$check_row) {
        ...
        header("Location: accountant_dashboard.php?page=complaints");
        exit();
    }
    ...
    if ($check_row['status'] === 'escalated') {
        ...
        $_SESSION['flash_type'] = "error";
        header("Location: accountant_dashboard.php?page=view-complaint&id=" . $complaint_id);
        exit();
    }
    ...
    if (!empty($message)) {
        ...
        mysqli_stmt_bind_param($insert_stmt, "iis", $complaint_id, $accountant_id, $message);
        ...
        mysqli_stmt_bind_param($assign_stmt, "ii", $accountant_id, $complaint_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($file['type'], $allowed_types)) {
            ...
            $new_filename = 'accountant_' . $accountant_id . '_' . time() . '.' . $extension;
    ...
    if (empty($errors)) {
        ...
        $update_stmt = mysqli_prepare($conn, $update_sql);
        mysqli_stmt_bind_param($update_stmt, "sssi", $new_full_name, $new_phone, $new_profile_picture, $accountant_id);
        mysqli_stmt_execute($update_stmt);
        mysqli_stmt_close($update_stmt);
        ...
        $_SESSION['flash_type'] = "success";
        header("Location: accountant_dashboard.php?page=profile");
        exit();
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_password'])) {
    ...
    if (empty($errors)) {
        ...
        mysqli_stmt_bind_param($pass_stmt, "si", $hashed, $accountant_id);
        mysqli_stmt_execute($pass_stmt);
        mysqli_stmt_close($pass_stmt);
        $_SESSION['flash_message'] = "Password changed successfully!";
        $_SESSION['flash_type'] = "success";
        header("Location: accountant_dashboard.php?page=change-password");
        exit();
    } else {
        $_SESSION['flash_message'] = implode("<br>", $errors);
        $_SESSION['flash_type'] = "error";
...
mysqli_stmt_bind_param($prof_stmt, "i", $accountant_id);
...
if ($table_check && mysqli_num_rows($table_check) > 0) {
    ...
    if ($ann_stmt) {
        ...
        mysqli_stmt_bind_param($ann_stmt, "ii", $dept_id, $accountant_id);
...
<head>
    ...
    <title>Accountant Dashboard - IAA CFMS</title>
...
<div class="sidebar" id="sidebar">
    <div class="sidebar-header">
        <div class="row-cfms">
            <span class="brand">CFMS <span>| Accountant</span></span>
            <button class="toggle-inline" id="toggleInline">❮</button>
        </div>
        <div class="row-tagline">
            <span class="tagline">Finance & Fees</span>
            <button class="toggle-standalone" id="toggleStandalone">❮</button>
        </div>
    </div>

    <div class="sidebar-menu">
        <a href="accountant_dashboard.php?page=dashboard" class="menu-item <?php echo $active_page == 'dashboard' ? 'active' : ''; ?>">
            <i class="fas fa-tachometer-alt"></i><span>Dashboard</span>
        </a>
        <a href="accountant_dashboard.php?page=complaints" class="menu-item <?php echo $active_page == 'complaints' ? 'active' : ''; ?>">
            <i class="fas fa-file-alt"></i><span>All Complaints</span>
        </a>
        <a href="accountant_dashboard.php?page=pending" class="menu-item <?php echo $active_page == 'pending' ? 'active' : ''; ?>">
            <i class="fas fa-clock"></i><span>Pending</span>
        </a>
        <a href="accountant_dashboard.php?page=resolved" class="menu-item <?php echo $active_page == 'resolved' ? 'active' : ''; ?>">
            <i class="fas fa-check-circle"></i><span>Resolved</span>
        </a>
        <a href="accountant_dashboard.php?page=escalated" class="menu-item <?php echo $active_page == 'escalated' ? 'active' : ''; ?>">
            <i class="fas fa-exclamation-triangle"></i><span>Escalated</span>
        </a>
        <a href="announcements.php" class="menu-item">
            <i class="fas fa-bullhorn"></i><span>Announcements</span>
        </a>
        <a href="accountant_dashboard.php?page=profile" class="menu-item <?php echo $active_page == 'profile' ? 'active' : ''; ?>">
            <i class="fas fa-user-circle"></i><span>Profile</span>
        </a>
        <a href="accountant_dashboard.php?page=change-password" class="menu-item <?php echo $active_page == 'change-password' ? 'active' : ''; ?>">
    ...
    <div class="top-bar">
        <div>
            <div style="font-size: 0.9rem; font-weight: 600; color: #0a2a5e;">
                <i class="fas fa-calculator" style="color: #1a56db;"></i> Welcome, <?php echo htmlspecialchars($accountant_name); ?>
            </div>
            <div style="font-size: 0.75rem; color: #6b85a0;">Accountant</div>
        </div>
        <div class="profile-info">
            <div class="profile-pic">
                ...
                <?php else: ?>
                    <?php echo strtoupper(substr($accountant_name, 0, 1)); ?>
                <?php endif; ?>
            </div>
            <div class="profile-details">
                <div class="name"><?php echo htmlspecialchars($accountant_name); ?></div>
                <div class="reg">Accountant</div>
            </div>
        </div>
    </div>
            ...
            <div class="dashboard-grid">
                ...
                <div class="content-area">
                    ...
                    <div style="margin-top: 15px; text-align: center;">
                        <a href="accountant_dashboard.php?page=complaints" class="btn-sm" style="background: #6b85a0;">View All Complaints <i class="fas fa-arrow-right"></i></a>
                    </div>
                </div>
        ...
        <?php elseif ($active_page == 'complaints' || $active_page == 'pending' || $active_page == 'resolved' || $active_page == 'escalated'): ?>
            ...
            <div class="content-area">
                            ...
                            <?php else: ?>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td><span class="badge <?php echo $status_class; ?>"><?php echo ucfirst($row['status']); ?></span></td>
                                        <td><?php echo date('d/m/Y', strtotime($row['created_at'])); ?></td>
                                        <td><a href="accountant_dashboard.php?page=view-complaint&id=<?php echo $row['id']; ?>" class="btn-sm">View</a></td>
                                    </tr>
                                <?php endwhile; ?>
                            <?php endif; ?>
                        </tbody>
                ...
                <div class="pagination">
                    <?php if ($page > 1): ?>
                        <a href="accountant_dashboard.php?page=<?php echo $active_page; ?>&search=<?php echo urlencode($search); ?>&priority=<?php echo $priority_filter; ?>&sort=<?php echo $sort; ?>&p=<?php echo $page-1; ?>"><i class="fas fa-chevron-left"></i></a>
                    <?php endif; ?>
                    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                        <?php if ($i == $page): ?>
                            <span class="current"><?php echo $i; ?></span>
                        <?php else: ?>
                            <a href="accountant_dashboard.php?page=<?php echo $active_page; ?>&search=<?php echo urlencode($search); ?>&priority=<?php echo $priority_filter; ?>&sort=<?php echo $sort; ?>&p=<?php echo $i; ?>"><?php echo $i; ?></a>
                        <?php endif; ?>
                    <?php endfor; ?>
                    <?php if ($page < $total_pages): ?>
                        <a href="accountant_dashboard.php?page=<?php echo $active_page; ?>&search=<?php echo urlencode($search); ?>&priority=<?php echo $priority_filter; ?>&sort=<?php echo $sort; ?>&p=<?php echo $page+1; ?>"><i class="fas fa-chevron-right"></i></a>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
            </div>
        ...
        <?php elseif ($active_page == 'view-complaint' && $complaint_id): ?>
            ...
            if (!$complaint) { 
                ...
                if (is_null($complaint['assigned_to'])) {
                    ...
                    mysqli_stmt_bind_param($auto_stmt, "ii", $accountant_id, $complaint_id);
                    mysqli_stmt_execute($auto_stmt);
                    mysqli_stmt_close($auto_stmt);
                    $complaint['assigned_to'] = $accountant_id;
                }
            ...
            <div class="content-area">
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="full_name" value="<?php echo htmlspecialchars($accountant_name); ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Email (cannot be changed)</label>
                        <input type="email" value="<?php echo htmlspecialchars($accountant_email); ?>" disabled style="background:#f0f4f9; cursor:not-allowed;">
                    </div>
                    <div class="form-group">
